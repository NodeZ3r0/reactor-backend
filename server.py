...
async def startup():
    ...
    await db.init_pool()
    # print('⚠ DB init skipped for Reactor IDE')  # TEMP DISABLED
    print("=" * 60)
...
async def proxy_mcp(path: str, request: Request):
    mcp_base = "https://mcp.wopr.systems"
    url = f"{mcp_base}/api/{path}"
...
@_ui_router.get("/health")
async def ui_health(api_key: str = Security(verify_api_key)):
    return await health()

@_ui_router.get("/models/status")
async def ui_models_status(api_key: str = Security(verify_api_key)):
    ...
    configured_env = os.getenv("CONFIGURED_MODELS", "").strip()
    configured = [m.strip() for m in configured_env.split(",") if m.strip()] if configured_env else available
    missing = [m for m in configured if m not in available]

    return {
        "status": "ok" if available else "unknown",
...
@_ui_router.get("/runs")
async def ui_runs(api_key: str = Security(verify_api_key)):
    async with db.pool.acquire() as conn:
        sessions = await conn.fetch("""
    ...
    for s in sessions:
        runs.append({
            "run_id": str(s["session_id"]),
            "created": s["created_at"].isoformat() if s["created_at"] else None,
            "input": {"task": s["task_description"], "type": "session"},
            "output": {"status": s["status"] or "in_progress"},
        })
    ...
    for d in docs:
        runs.append({
            ...
            "input": {"task": f"doc:{d['source']}", "type": "document", "source": d["source"]},
            "output": {"status": "ingested"},
        })

    runs.sort(key=lambda x: x.get("created") or "", reverse=True)
    return {"runs": runs}

@_ui_router.post("/rag/ingest")
async def ui_rag_ingest(payload: dict = Body(...), api_key: str = Security(verify_api_key)):
    text = (payload.get("text") or "").strip()
    source = (payload.get("source") or "unknown").strip()
    metadata = payload.get("metadata")

    if not text or not source:
        raise HTTPException(status_code=400, detail="Missing text or source")
...
@_ui_router.post("/pipeline/run")
async def ui_pipeline_run(payload: dict = Body(...), api_key: str = Security(verify_api_key)):
    return await run_pipeline(payload, api_key=api_key)
